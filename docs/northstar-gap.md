...
- После #268 execution-mode routing, daemon selection/claims и fresh-branch one-shot issue path уже принадлежат Go; Python runner больше не должен скрыто переопределять эти решения.
...
- нет Go-owned execution-mode routing между issue-flow и PR-review compatibility adapter.
...
### 3. Оставшийся runtime migration хвост в Python adapter

Go CLI и часть orchestration runtime уже перенесены, но несколько execution loops все еще живут в Python compatibility adapter.

Остается сделать:

- перенести PR review runtime и merge progression loop из adapter в Go без потери текущего внешнего поведения;
- перенести reused-branch/conflict-recovery execution path из issue adapter в Go;
- убрать зависимость daemon/batch worker execution от Python script там, где Go уже владеет selection/state policy.
...
3. Продолжить перенос оставшихся PR/reuse worker runtimes из Python adapter в Go.
4. Вынести provider abstraction из GitHub-centric runtime.

## Итог

Главный оставшийся разрыв с North Star больше не в отсутствии базовой оркестрации или ownership guardrails. Он сместился в четыре конкретные зоны: реальный smoke execution для автономного режима, autonomous merge queue, оставшийся Python adapter runtime хвост и provider-agnostic architecture.
