...
class AutonomousDaemonSelectionTests(unittest.TestCase):
    ...
    def test_main_autonomous_batch_skips_recovered_blocked_issue_and_continues(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            session_path = os.path.join(temp_dir, "daemon-session.json")
            args = argparse.Namespace(
                repo="owner/repo",
            ...
            blocked_issue = {
                ...
                "title": "Blocked issue",
                "body": "non-empty",
                "url": "https://github.com/owner/repo/issues/254",
                "labels": [{"name": "bug"}],
                "assignees": [],
                "createdAt": "2026-04-26T10:00:00Z",
                "updatedAt": "2026-04-28T10:00:00Z",
            ...
            linked_pr = {
                "number": 140,
                "url": "https://github.com/owner/repo/pull/140",
                "headRefName": "issue-fix/247-runnable-issue",
                "baseRefName": "main",
                "reviews": [],
                "author": {"login": "dev"},
                "mergeStateStatus": "CLEAN",
            }
            blocked_state_comment = {
                "id": 1,
            ...
            with (
                ...
                patch(
                    ...
                    return_value=[blocked_issue, runnable_issue],
                ),
                patch(
                    "scripts.run_github_issues_to_opencode.sort_autonomous_issues",
                    return_value=[blocked_issue, runnable_issue],
                ),
                patch(
                    "scripts.run_github_issues_to_opencode.split_autonomous_issues_by_dependency_state",
                    return_value=([blocked_issue, runnable_issue], []),
                ),
                patch(
                    "scripts.run_github_issues_to_opencode.find_open_pr_for_issue",
                    side_effect=[None, linked_pr],
                ),
                patch(
                    "scripts.run_github_issues_to_opencode.fetch_issue_comments",
                    side_effect=[[blocked_state_comment], [], [pr_state_comment]],
                ),
                patch("scripts.run_github_issues_to_opencode.fetch_pull_request", return_value=linked_pr),
                patch("scripts.run_github_issues_to_opencode.fetch_pr_review_threads", return_value=[]),
                patch("scripts.run_github_issues_to_opencode.fetch_pr_conversation_comments", return_value=[]),
            ...
            run_agent_mock.assert_not_called()
            output = stdout_mock.getvalue()
            self.assertIn("Skipping issue #254: recovered orchestration state is blocked", output)
            self.assertIn("Auto-switch to PR-review mode", output)
            self.assertIn("Done. Processed: 1", output)
            self.assertIn("skipped_recovered_state: 1", output)

            saved_state = load_autonomous_session_state(session_path)
            self.assertEqual(saved_state["checkpoint"]["counts"]["processed"], 1)
            self.assertEqual(saved_state["checkpoint"]["counts"]["skipped_recovered_state"], 1)
